...
text_transform = {}
for ln in [SRC_LANGUAGE, TGT_LANGUAGE]:
    text_transform[ln] = sequential_transforms(tokenize,
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    if epoch % 3 == 0:
      ...
      for pr in d_pairs:
        ...
        targets.append(translate(transformer, pr[0]))
      bleu_score_corpus = corpus_bleu(refs, targets)
...
translate(transformer, "je n en suis pas trop convaincu .")

test_lines = open("./data/fren.test.bpe", encoding='utf-8').readlines()
fi = open("translations_1", "w")
count = 0
for line in test_lines:
  count += 1
  fi.write(translate(transformer, line.strip().split("|||")[0]).strip()+"\n")
print(count)