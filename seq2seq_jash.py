...
class LSTM(nn.Module):
    def __init__(self, hidden_size, cv):
        ...
        self.f_gate = nn.Linear(hidden_size * num, hidden_size)
        self.i_gate = nn.Linear(hidden_size * num, hidden_size)
        self.i_c_gate = nn.Linear(hidden_size * num, hidden_size)
        self.o_gate = nn.Linear(hidden_size * num, hidden_size)

        self.o_ll = nn.Linear(hidden_size, hidden_size)

        self.c_state = torch.zeros(1, 1, hidden_size, device=device)

    def forward(self, i_state, hidden, cv=None):
        
        if cv is not None:
            input = torch.cat([i_state, hidden, cv], dim=-1)  
        else:
            input = torch.cat([i_state, hidden], dim=-1)
        
        f_state = torch.sigmoid(self.f_gate(input)) 

        i_state = torch.sigmoid(self.i_gate(input))  
        i_c_state = torch.tanh(self.i_c_gate(input)) 
        o_state = torch.sigmoid(self.o_gate(input))  

        cell = (f_state * self.c_state) + (i_state * i_c_state)

        h_state = o_state * torch.tanh(cell)  
